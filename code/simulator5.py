...
def simulate(lc):
    ...
    sims=[]
    # print(parameters)
    # for p1 in parameters[:,0]:
    #     for p2 in parameters[:,1]:
    #         for p3 in parameters[:,2]:
    #             for p4 in parameters[:,3]:
    #                 for p5 in parameters[:,4]:
...
def normalize(lc):
    ...
    for i in range(len(lc)):
        lc['adjflux'][i]=np.random.normal(0, lc['fluxerr'][i])
...
def process_batch(batch):
    ...
    for id in tqdm(batch):
        ...
        for (i, (sim, parameters)) in enumerate(sims):
            ascii.write(sim, f'{simulations_path}/data/{id}_{i}.dat', overwrite=True)
            flare= process_light_curve(f'{id}_{i}', sim, adjust_parameters=False, reset_params=True, show = False, save = True, save_pickle=True, plot_std=False, fig_paths=sim_fig_paths, pickle_paths=sim_pickle_paths)
            simulations_info.add_row([f'{id}_{i}', parameters[0], parameters[1], parameters[2], parameters[3], parameters[4], flare])
