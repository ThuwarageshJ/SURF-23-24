...
class LightCurve:
    ...
    def plot(self, save_loc: str = None, regress_before_plotting : bool = True, show: bool =False, save: bool = False)-> None:
        ...
        for filter in self.filters:
            ...
            if len(self.thiel_sen_prediction[filter])!=0:
    ...
    def regress(self):
        ...
        for filter in self.filters:
            if len(self.data[filter])==0:
                continue
            kernel = Matern32Kernel(1.0)
            gp = george.GP(kernel, mean=np.mean(self.data[filter]), fit_mean=True,
               white_noise=np.log(np.square(self.dataerr[filter])), fit_white_noise=True)
